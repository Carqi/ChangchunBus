...
public class NetListActivity extends MapActivity {
	...
	public class MySearcherListener implements MKSearchListener {
		...
		public void onGetTransitRouteResult(MKTransitRouteResult res, int iError) {
			if (iError != 0 || res == null) {
				Toast.makeText(NetListActivity.this, "抱歉，未找到结果",
			...
			for(int i=0 ; i<res.getNumPlan() ; i++){
				...
				String strcontent = "";
				for(int j=0 ; j<routePlan.getNumLines() ; j++){
					mkLine = routePlan.getLine(j);
					if (j == routePlan.getNumLines() - 1) {
						strcontent = strcontent + mkLine.getTitle();
					} else {
						strcontent = strcontent + mkLine.getTitle() + "→";
					}
				}
				Bus bus = new Bus();
				bus.setLine(strcontent);
				busList.add(bus);
			}
